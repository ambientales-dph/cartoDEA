...
export function usePortalAuth() {
  ...
  useEffect(() => {
    ...
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
      if (!auth.currentUser) {
        console.log("Modo Desarrollo: Esperando interacción o sesión activa.");
      }
      return;
    }

    // 2. Recuperar el Custom Token de la cookie de intercambio (set por el backend tras el POST)
